...
> 人類可讀的繁體中文版本；AI 工具請改讀 canonical 版本 [requirements-en.md](requirements-en.md)。本 repo 閱讀順序：requirements-en.md → readme-en.md → 其他文件。
...
**範疇內**：依 Keep2Share 分享連結下載單一檔案；透過 captcha 驗證通過 Keep2Share 公開 API 授權；並行分段傳輸並具重試／backoff；可選的 proxy pool 以取得 IP 多樣性；下載後可選的 `ffmpeg` 媒體完整性檢查。

**範疇外**：單次執行下載多個 URL 的批次／佇列功能；Keep2Share 付費帳號等級的存取；跨 process 執行的斷點續傳（若磁碟上沒有既有的 `.partNNN` 檔案則無法接續）；任何 Keep2Share 上傳功能；繞過或自動化解 captcha（本工具是把 captcha 交給使用者／呼叫端處理，見 REQ-5）。
...
- **AC-3.5**：磁碟上已存在且大小符合預期的 `.partNNN` 檔案會直接重複使用、不重新下載（在同一次 `download()` 呼叫的暫存目錄內具備當機／重啟復原能力）。
- **AC-3.6**：成功後所有 part 檔案會依區段順序串接成最終目標檔案，並刪除暫存 part 檔；目標路徑上若已有舊檔會被覆寫。
...
- **NFR-1（Timeout）**：每一個對外網路呼叫都有明確命名的 timeout 常數（見 readme-en.md 的「Timeout inventory」），不依賴 library 預設值。
