...
> Canonical, AI-facing architecture document. Human-facing Traditional Chinese counterpart: [readme-zh.md](readme-zh.md). Reading order for this repo: [requirements-en.md](requirements-en.md) → this file → other docs. For setup/usage instructions see the human-facing [Readme.md](Readme.md); for contribution workflow see [CONTRIBUTING.md](CONTRIBUTING.md).
...
Security posture (see also the Readme's "Security Note: Public Proxies" section): the proxy hop itself is unauthenticated plain HTTP even when the target URL is HTTPS (`requests` tunnels HTTPS through an HTTP `CONNECT` to the proxy), so a malicious proxy operator is positioned to observe or tamper with routed traffic. This is a deliberate, documented tradeoff for working around per-IP rate limits, not an oversight — direct connection is always preferred (§ 3) and the proxy pool is purely a fallback.
...
Retry/backoff parameters (module-level constants in `downloader.py` / `k2s_client.py`, intentionally centralized rather than inlined — see `todolist.md` P2-2):
