# K2S Downloader — Todolist 封存：第一輪（P0 ~ P5）

> **本檔為封存內容，非目前的 active backlog。** 目前的 backlog 請見 [`../todolist.md`](../todolist.md)。
> 這是第一輪（P0 最嚴重／會損毀資料或掛死 → P5 文件與 DX）review 產生的全部項目，已於 2026-07-16 ~
> 2026-07-17 全數完成並合併進 `main`，依 [R2-15](../todolist.md#r2-p6) 的歸檔規則搬移至此，僅供查閱
> 歷史脈絡（例如某個既有設計決策當初為何這樣做）。技術/程式術語保留英文；敘述以繁體中文（台灣用語）為主。
...
>
> **第一輪 P0 ~ P5 六個優先級皆已全數完成。** 2026-07-17 依「重新從頭檢視現況」原則展開第二輪 review，
> 產生的新 backlog（R2-x）見 [`../todolist.md`](../todolist.md)。
