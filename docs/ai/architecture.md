...
| `DownloadIntegrityError` | A part file no longer matches its range's expected size at merge time, or the merged file's size doesn't match the sum of all ranges | "The bytes on disk are not the file you asked for." A `RuntimeError` subclass, so existing "download failed" handling still catches it; distinct so a caller can tell a local/data-integrity failure apart from a network one. A part mismatch leaves every part and the manifest in place (the next run resumes and re-fetches just that segment); a merged-size mismatch deletes the incomplete output instead of returning it (R3-3). |
