...
- **AC-4.1**: A chunk that fails (non-2xx HTTP status, network-level exception, or a byte-count mismatch) is retried with exponential backoff (base 1s, capped at 30s) up to a fixed maximum attempt count (8), after which this attempt at the download raises a `ChunkDownloadFailed` naming the range, attempt count, and last recorded failure reason.
...
- **AC-4.5**: A `ChunkDownloadFailed` (AC-4.1) does not abort the whole `download()` call outright: it's caught and the whole download is automatically re-attempted (fresh captcha, fresh URLs) up to `MAX_DOWNLOAD_RETRIES` (3) total attempts before the failure is finally raised to the caller. Because REQ-11's resume manifest is consulted again on each re-attempt, already-completed ranges are skipped rather than re-downloaded, so a retry only redoes the range(s) that actually got stuck. Cancellation (`DownloadCancelled`) and any other exception type are never retried this way — only `ChunkDownloadFailed` is.
