...
- **AC-4.1**: A chunk that fails (non-2xx HTTP status, network-level exception, or a byte-count mismatch) is retried with exponential backoff (base 1s, capped at 30s) up to a fixed maximum attempt count (`MAX_CHUNK_RETRIES`, 25), after which the whole download aborts with a `ChunkDownloadFailed` naming the range, attempt count, and last recorded failure reason. `download()` deliberately does *not* auto-retry the whole download after this (see R2-16 in `docs/ai/todolist.md`): a prior attempt at that made things worse in practice, forcing a fresh captcha and a full proxy-cycling search per retry against a large, mostly-dead pool. Retrying within one chunk needs neither.
