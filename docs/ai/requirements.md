...
- **AC-3.4**: A range whose downloaded byte count doesn't match its expected size **exactly** is treated as failed and retried, not silently accepted. The count is checked twice — against the bytes handed to `write()` and against the size of the part file itself — because a byte count that is off by even one shifts every following byte of the merged file (R3-1: this check previously allowed a 1-byte tolerance, which produced a file that still played as video but could never be extracted as an archive).
...
- **AC-3.7**: A chunk only accepts a response that describes the range it asked for: a `206`'s `Content-Range` must name exactly the requested first/last byte, and a `200` (meaning the server ignored `Range` and is sending the whole file from offset 0) is rejected outright for any split of more than one range. A `206` whose `Content-Range` is absent or unparseable is accepted on its byte count alone. Rationale (R3-2): a response of the right length but the wrong offset is otherwise indistinguishable from success and splices foreign bytes into the middle of the output.
- **AC-3.8**: Before the merge writes a single byte, every part file is re-verified against its range's expected size; a mismatch aborts with `DownloadIntegrityError` while leaving all parts and the manifest untouched, so the next run resumes and re-fetches only the damaged segment. After the merge, the finished file's size must equal the sum of all ranges; if it doesn't (a short write — full disk, failing drive), the incomplete output is deleted rather than handed back as if it were the file (R3-3).

### REQ-4: Bounded retry with backoff, not indefinite hangs
- **AC-4.1**: A chunk that fails (non-2xx HTTP status, a response that isn't the requested range, a network-level exception, or a byte-count mismatch) is retried with exponential backoff (base 1s, capped at 30s). The budget (`MAX_CHUNK_RETRIES`, 25) counts **consecutive failures during which the download as a whole made no progress** — any byte received or range credited anywhere restarts it (R3-7). Only when that budget is exhausted does the whole download abort, with a `ChunkDownloadFailed` naming the range, the consecutive-failure count, and the last recorded failure reason. Rationale: as a lifetime counter it aborted multi-hour downloads over ordinary accumulated bad luck while the other 499 segments were completing normally; measured against progress it still stops a genuinely stuck download (nothing moving anywhere, so nothing restarts the budget) in the same ~10 minutes. `download()` deliberately does *not* auto-retry the whole download after this (see R2-16 in `docs/ai/todolist.md`): a prior attempt at that made things worse in practice, forcing a fresh captcha and a full proxy-cycling search per retry against a large, mostly-dead pool. Retrying within one chunk needs neither.
...
- **AC-4.5**: Each download-URL slot carries its own consecutive-failure count; crossing `URL_SLOT_FAILURE_THRESHOLD` benches that slot for `URL_SLOT_COOLDOWN_SECONDS` and a success clears it, and the scheduler picks slots from a rotating cursor rather than always scanning from index 0 (R3-6). Rationale: Keep2Share binds each issued URL to the first IP that uses it, so one slot can die while the rest work — and because a broken slot fails in a second while a healthy one is minutes into a segment, a scan from index 0 handed nearly every retry to the broken slot.
...
- **AC-10.7**: When the user finishes entering a URL, the GUI checks (locally, no network call) whether a resume manifest exists for that file's ID and, if so, displays the previous run's completion percentage and byte counts before the download is even started; entering a URL with no matching manifest, or an invalid URL, hides this hint instead of showing stale or incorrect information.
...
- **AC-11.5**: A manifest's progress can be queried by Keep2Share file ID alone (`Downloader.find_resume_progress`), without needing the resolved filename in advance (which would require an API call) — this is what lets a caller preview "you're already partway through this file" before starting a download at all (see AC-10.7).
...
- **NFR-6 (Bounded chunk memory)**: a chunk's data is streamed straight to its `.partNNN.tmp` file as it arrives (not buffered whole in memory) and flushed after every write so it is visible on disk incrementally, not only once the chunk completes; the rename to its final `.partNNN` name is atomic and only happens once the full expected byte count is confirmed, so an in-progress or crashed attempt can never be mistaken for a complete part. The part file is `fsync`ed before that rename, and the manifest before its own atomic replace, so a crash or power loss cannot leave a part with the right length but unwritten (zero-filled) contents for the resume path — which has only the length to go on — to accept as good (R3-5).
