...
- **AC-3.4**：下載完成的位元組數與預期大小**完全相等**才算成功，否則視為失敗並重試，不會被靜默接受。位元組數會檢查兩次——一次是交給 `write()` 的累計量，一次是 part 檔本身的實際大小——因為只要差一個 byte，合併後所有後續位元組都會位移（R3-1：此檢查原本容許 1 byte 誤差，產出的檔案影片仍可播放，但壓縮檔永遠解不開）。
...
- **AC-3.7**：區段只接受「描述的範圍正是自己要求的範圍」的回應：`206` 的 `Content-Range` 必須恰好等於請求的起訖位元組；`200`（代表伺服器忽略 `Range`、從 offset 0 送整個檔案）在切分超過一段時一律拒收。若 `206` 沒有帶 `Content-Range` 或無法解析，則僅依位元組數判定。理由（R3-2）：長度正確但 offset 錯誤的回應與成功完全無法區分，會把不屬於這裡的位元組接到輸出檔中間。
- **AC-3.8**：合併寫出第一個位元組之前，會先逐一重新驗證每個 part 檔的大小是否符合其區段；不符則以 `DownloadIntegrityError` 中止，且**不動**任何 part 檔與 manifest，讓下次執行續傳時只重抓損毀的那一段。合併完成後，最終檔案大小必須等於所有區段大小的總和；若不符（寫入不完整——磁碟滿、磁碟機故障），不完整的輸出檔會被刪除，而不是當成正常檔案交還（R3-3）。

### REQ-4：有界重試與 backoff，而非無限期卡住
- **AC-4.1**：失敗的區段（非 2xx 狀態碼、回應描述的不是所要求的 range、網路層例外、或位元組數不符）會以指數 backoff（基數 1 秒、上限 30 秒）重試。重試預算（`MAX_CHUNK_RETRIES`，25 次）計算的是**「整個下載毫無進度期間的連續失敗次數」**——任何區段收到任何位元組或被記為完成，都會讓預算重新開始（R3-7）。只有預算耗盡才會中止整個下載，並丟出 `ChunkDownloadFailed`（附區段編號、連續失敗次數與最後一次失敗原因）。理由：當它是「整趟累計」時，跑好幾小時的下載會因為平常的運氣不好而被中止，而其餘 499 段其實都正常完成中；改成以進度為基準後，真正卡死（任何地方都沒有進度，所以預算不會重置）的下載仍然一樣在約 10 分鐘內停下來。`download()` 依然**不會**在這之後自動重跑整個下載（見 `docs/ai/todolist.md` R2-16）：先前試過那麼做，體驗反而更糟——每次自動重試都要重新解一次 captcha，還要把現在（R2-10 後）又大又混雜死 proxy 的候選清單重新掃一遍。
...
- **AC-4.5**：每個下載 URL slot 都有自己的連續失敗計數；超過 `URL_SLOT_FAILURE_THRESHOLD` 就把該 slot 冷卻 `URL_SLOT_COOLDOWN_SECONDS` 秒，成功則清除計數；排程器改由一個輪替游標挑 slot，不再永遠從 index 0 開始掃（R3-6）。理由：Keep2Share 發出的每支 URL 會綁定第一個使用它的 IP，所以單一 slot 可能壞掉而其餘正常——而壞掉的 slot 一秒內就失敗、健康的 slot 卻要好幾分鐘才跑完一段，從 index 0 開始掃等於把幾乎每一次重試都送給壞掉的那支。
...
- **AC-10.7**：使用者輸入完 URL 後，GUI 會（純本機檢查，不打任何網路請求）確認該檔案 ID 是否已有續傳 manifest，若有，會在下載開始前就顯示上次的完成百分比與位元組數；若 URL 沒有對應的 manifest、或 URL 本身無效，則會隱藏這個提示，不會顯示過期或錯誤的資訊。
...
- **AC-11.5**：manifest 的進度可以只憑 Keep2Share 檔案 ID 查詢（`Downloader.find_resume_progress`），不需要事先知道解析後的檔名（那需要一次 API 呼叫）—— 這讓呼叫端能在下載都還沒開始前，就先預覽「這個檔案你已經下載到一半了」（見 AC-10.7）。
...
- **NFR-6（區塊記憶體用量有界）**：區段資料一邊接收一邊直接串流寫入其 `.partNNN.tmp` 檔（不會整段緩衝在記憶體中），且每次寫入後都會 flush，讓磁碟上的檔案漸進成長可見，而不是等到整段下載完才出現；重新命名成最終 `.partNNN` 名稱的動作是原子性的，且只在確認完整位元組數後才執行，因此下載到一半或中斷的嘗試絕不會被誤判為已完成的區段。該 rename 之前會先對 part 檔做 `fsync`，manifest 在原子性覆寫前也一樣，避免當機或斷電後留下「大小正確但內容未實際寫入（為 0）」的 part 檔——續傳路徑只能依大小判斷，會把它當成好的（R3-5）。
