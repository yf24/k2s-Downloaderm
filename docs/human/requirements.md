...
- **AC-4.1**：失敗的區段（非 2xx 狀態碼、網路層例外、或位元組數不符）會以指數 backoff（基準 1 秒、上限 30 秒）重試，最多固定次數（`MAX_CHUNK_RETRIES`，25 次），超過後整個下載會中止並丟出 `ChunkDownloadFailed`，內含區段編號、嘗試次數與最後一次失敗原因。`download()` 刻意**不會**在這之後自動重跑整個下載（見 `docs/ai/todolist.md` R2-16）：先前試過這麼做，實測反而更糟——每次自動重試都要重新解一次 captcha，還要對著現在（R2-10 後）又大又多半是死的 proxy 池整個重新跑一輪搜尋。單一區段內部重試不需要這兩者，所以是更好的解法。
